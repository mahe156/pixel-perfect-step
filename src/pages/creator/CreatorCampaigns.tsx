  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)} className="border-border text-muted-foreground">
          ...
          <div className="flex gap-1">
            {["all", "youtube", "instagram"].map((p) => (
              <button key={p} onClick={() => setPlatformFilter(p)}
                className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${platformFilter === p ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"}`}>
      ...
      {showFilters && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="glass rounded-xl p-5 space-y-4">
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-2">Niche</p>
            <div className="flex flex-wrap gap-2">
              {NICHES.map((n) => (
                <button key={n} onClick={() => toggleNiche(n)}
                  className={`badge-pill text-xs border transition-all ${selectedNiches.includes(n) ? "bg-primary/20 text-primary border-primary/30" : "bg-muted text-muted-foreground border-border"}`}>{n}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {LANGUAGES.map((l) => (
                <button key={l} onClick={() => toggleLang(l)}
                  className={`badge-pill text-xs border transition-all ${selectedLanguages.includes(l) ? "bg-primary/20 text-primary border-primary/30" : "bg-muted text-muted-foreground border-border"}`}>{l}</button>
              ))}
            </div>
          </div>
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-2">CPM Range: ₹{cpmRange[0]} — ₹{cpmRange[1]}</p>
      ...
      {isLoading ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (<div key={i} className="glass rounded-xl p-6 h-64 shimmer" />))}
        </div>
      ) : filtered.length > 0 ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c, i) => (
            <motion.div key={c.id} className="glass-hover rounded-xl overflow-hidden cursor-pointer"
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              onClick={() => navigate(`/creator/campaigns/${c.id}`)}>
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-bold text-foreground">{c.title.charAt(0)}</div>
                  <span className={`badge-pill border text-xs flex items-center gap-1 ${platformColor[c.platform]}`}>{platformIcon[c.platform]}</span>
                </div>
                <h3 className="font-display font-bold text-foreground text-sm mb-2 line-clamp-2">{c.title}</h3>
                <div className="text-xl font-bold text-primary font-mono mb-3">₹{c.cpm_rate}<span className="text-xs font-normal text-muted-foreground"> /1K views</span></div>
                <div className="mb-3">
                  <div className="flex justify-between text-[10px] text-muted-foreground mb-1">
                    <span>Budget</span>
                    <span>{Math.round(((Number(c.spent_amount) || 0) / Number(c.total_budget)) * 100)}%</span>
                  </div>
                  <Progress value={((Number(c.spent_amount) || 0) / Number(c.total_budget)) * 100} className="h-1 bg-muted" />
                </div>
